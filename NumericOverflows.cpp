...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        if (result >= 0 && increment > std::numeric_limits<T>::max() - result) {
            std::cout << "Stopped on Iteration " << i << " With result value before operation: " << result << std::endl;
            throw "ERROR: Addition failed due to numeric overflow.";
        }
        result += increment;
    }
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        if (result < std::numeric_limits<T>::min() + decrement) {
            std::cout << "Stopped on Iteration " << i << " With result value before operation: " << result << std::endl;
            throw "ERROR: Subtraction failed due to numeric underflow.";
        }
...
void test_underflow()
{
    ...
    std::cout << "\tSubtracting Numbers With Overflow (" << +start << ", " << +decrement << ", " << (steps + 10) << ") = ";
    try {
        T result = subtract_numbers<T>(start, decrement, steps + 10);
        std::cout << +result << std::endl;
    }
    catch (const char* err) {
        std::cerr << err << std::endl;
